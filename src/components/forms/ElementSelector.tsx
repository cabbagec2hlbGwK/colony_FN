...
export const ElementSelector: React.FC<ElementSelectorProps> = ({ 
  ...
  const { rawElements } = useElementStore();
  ...
  const handleElementSelect = (key: string, count: number) => {
    ...
    // Toggle summary view
    if (selectedFilter?.selector === element.selector && 
        selectedFilter?.type === element.type) {
          ...
          {filteredElements.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200">
              {filteredElements.map(([key, info]) => {
                ...
                const isFiltered = selectedFilter?.selector === selector && 
